...
<template>
  <div class="max-w-260px h-full min-h-780px bg-white">
    <div class="flex items-center h-34px">
      ...
      <el-input style="flex: 2" size="small" v-model="searchValue" placeholder="请输入部门名称" clearable>
        <template #suffix>
      ...
      <el-dropdown>
        <IconifyIconOffline class="w-28px cursor-pointer" width="18px" icon="more-vertical" />
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item>
              <el-button
    ...
    >
      <template #default="{ node, data }">
        ...
        >
          <IconifyIconOffline
            :icon="data.type === 1 ? 'office-building' : data.type === 2 ? 'location-company' : 'dept'"
          />
          {{ node.label }}
        </span>
      </template>
